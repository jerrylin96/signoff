...
- [ ] Phase 4 (Extraction & Distribution): dedicated signoff GitHub repository (history carried via `git filter-repo`), consumed by this repo via submodule or subtree; project website (GitHub Pages first). End-user distribution requires no repo linking: Claude Code plugin marketplace as primary channel (user-scope installs on CLI/desktop; repo-declared `.claude/settings.json` plugins install at session start on web and local), claude.ai skill upload as the user-scope channel for web sessions (user-installed plugins do not transfer to cloud sessions), and self-contained folder copy for other harnesses per `skills/signoff/HARNESSES.md`.
