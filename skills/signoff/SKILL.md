...
When `/signoff` is run without an explicit intensity flag (`--quick` / `--deep`), the agent dynamically inspects the range diff to auto-select the Adaptive Interview Intensity level based on semantic impact and blast radius:
...
2. **High-Impact Content/Path Triggers:** If diff touches security/auth (`auth/`, `crypto/`, `permissions/`, secrets), schemas/migrations (`migrations/`, `schema.sql`, `ALTER TABLE`, ORM models), public API contracts (`*.proto`, OpenAPI specs), or scientific computation (scientific stack, notebooks, physical constants, solvers), auto-select Tier 2 (`skeptical`). Docs-only capping does not apply if executable/schema paths are touched.
3. **Pure Documentation / Comment Capping:** Diffs consisting solely of pure documentation or comments (`*.md`, docs, docstrings, formatting/lint, type annotations) cap at max Tier 1 (`standard`), even if exceeding >5 files or >200 lines. Path tokens like `auth/` or `migrations/` do not trigger Tier 2 on `*.md` files alone.
4. **Executable Code Blast Radius:** Non-boilerplate executable code changes exceeding >5 files or >200 lines auto-select Tier 2 (`skeptical`). Non-boilerplate code excludes lockfiles, generated stubs/protos, and license headers.
5. **Tiny Pure Documentation / Types (Tier 0):** Diffs with <50 LoC AND ≤2 files of pure documentation/types with zero high-impact triggers auto-select Tier 0 (`cursory`). Pure documentation diffs between 50–200 LoC (or 3–5 files) classify as Tier 1 (`standard`).
6. **Else (Default Feature Work):** All other normal feature work, internal bugfixes, and non-breaking logic auto-select Tier 1 (`standard`).
...
- **Modifier Precedence & Safety Clamps:**
  ...
  - `/signoff --quick`: Requests Tier 0 (`cursory`). Permitted on small routine feature work or docs diffs that would otherwise classify as Tier 1, provided zero Tier 2 triggers are present. **One-way safety clamp:** --quick is strictly blocked and the agent MUST refuse cursory (auto-escalating to Tier 1 for blast radius on docs, or Tier 2 for high-impact content/path triggers) when any Tier 2 high-impact trigger is present.
...
- **Science-detection escalation (additive, on by default):** if the range diff touches scientific computation — scientific-stack imports (e.g. numpy, scipy, jax, torch, astropy, pandas, xarray), notebooks (`.ipynb`), RNG seeding, physical constants or unit-bearing quantities, numerical solvers/integrators, or dataset/model-config files (e.g. netCDF, GRIB, zarr) — the agent MUST announce the escalation, auto-select Tier 2 (`skeptical`), and apply the domain emphases of [profiles/domain-science.md](profiles/domain-science.md) additively on top of the active profile: at least two science-emphasis probes at skeptical (satisfied by Tier 2's two-per-axis requirement, with the science emphases applied). Additive only — it never replaces the active profile, removes axes, or lowers pass criteria. A science-flagged diff counts as "areas flagged by the active interview profile" for cursory eligibility, so cursory MUST be refused.
- **Documentation Churn Capping:** Diffs consisting solely of pure documentation or comments (`*.md`, docs, docstrings) exceeding >5 files or >200 lines cap at Tier 1 (`standard`) and do not trigger Tier 2 (`skeptical`).
...
- `/signoff --quick`: **cursory** intensity (Tier 0) — subject to the one-way safety clamp (strictly blocked and auto-escalated to standard/skeptical on high-impact diffs: `auth/`, `crypto/`, `permissions/`, `migrations/`, `schema.sql`, `ALTER TABLE`, `proto`, `OpenAPI`, scientific computation, or >5 files or >200 lines).
